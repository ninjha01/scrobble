...
def word_length_func():
    return 0


def pull_score_dict(round_str) -> Dict[str, int]:
    url = f"https://wordsolver.net/solvewords.php?arg=%23!q%3D{round_str}%26f%3D%26ftype%3Df_none%26dic%3Dd_twl%26type%3Dst_anagram%26ml%3D15%26ne%3D1%26cb%3D29767902096514387&_=1589737771353"
    response = requests.get(url)
    response_json = json.loads(response.content[19:])
    length_word_map = response_json["m"]

    word_length_dict = defaultdict(word_length_func)
